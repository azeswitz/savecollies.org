...
def run_in_cloned_git
  Dir.mktmpdir("cri-website-deploy") do |dir|
    sh "git clone -l --single-branch -s . #{dir}"
    Dir.chdir(dir) do
      sh "echo `pwd`"
      sh "if (git remote | grep -q '^github$'); then git remote remove github; fi"
      sh "git remote add github https://github.com/azeswitz/savecollies.org.git"
      sh "git fetch github"
      yield dir
    end
  end
...
task :build do
  sh "sass assets/custom/scss/style.scss assets/custom/css/style.css"
